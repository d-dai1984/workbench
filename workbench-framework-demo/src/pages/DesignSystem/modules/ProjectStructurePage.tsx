  ...
  return (
    <div className="skill-ref-page">
      ...
      <Row gutter={16}>
        <Col xs={24} lg={12}>
          <Card
            title={<Bi en="New Page Flow" cn="新建页面流程" />}
          ...
          >
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {CHECKLIST.map((item) => (
                ...
                >
                  <Tag color="blue" style={{ margin: 0, flexShrink: 0 }}>{item.step}</Tag>
                  <div style={{ flex: 1 }}>
                    <Text strong style={{ fontSize: 13 }}>{item.action}</Text>
                    <Text type="secondary" style={{ fontSize: 12, marginLeft: 6 }}>{item.actionCn}</Text>
                    <div>
                      <Text code style={{ fontSize: 11 }}>{item.detail}</Text>
                    </div>
                  </div>
                </div>
              ))}
      ...
      {/* Business Line Index */}
      <Card
        title={<Bi en="Business Line Index" cn="业务线索引" />}
        style={{ borderRadius: 16 }}
      >
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          {ROUTES.map((r) => (
            ...
            >
              <Tag color="blue" style={{ margin: 0, minWidth: 120, textAlign: 'center' }}>
                {r.line}
              </Tag>
              ...
              <Text code style={{ fontSize: 11, color: 'rgba(0,0,0,0.45)' }}>
                {r.nav}
              </Text>
            </div>
          ))}
          ...
          <Col xs={24} lg={12}>
            ...
            >
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                ...
                />
                <BiBlock
                  en="Changes to core/theme/ or core/config/ (global impact)"
                  cn="修改 core/theme/ 或 core/config/（全局影响）"
                />
              </div>
            </div>
          </Col>
