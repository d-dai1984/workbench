/**
 * CampaignBuilderPageAntD — AntD 标准布局规范版本
 *
 * 核心差异：
 * - 单列线性布局（无 sidebar）
 * - 所有 Section 纵向堆叠 Card
 * - Campaign Goals 下沉为独立 Card，与其他 Section 平级
 * - 全局 max-width: 1200px，与 dashboard 容器对齐
 * - Form layout="vertical"，Row gutter={[16,16]} 控制多列字段
 * - Col 使用 xs/md/lg 断点，支持响应式收缩
 */
import {
...
import '../styles/CampaignBuilderPageAntD.css'
...
export function CampaignBuilderPageAntD() {
  ...
  const handleFinish = (values: Record<string, unknown>) => {
    if (values.level === 'S') { setConflictVisible(true); return }
    setView('success')
  }
  ...
  return (
    <div className="cpb-antd-page">
      <div className="cpb-antd-container">

        {/* ── Page Header ─────────────────────────────────────────────────── */}
        <div className="cpb-antd-header">
          <Breadcrumb
            items={[
              { title: 'Promo System' },
          ...
          />
          <Title level={4} style={{ margin: '12px 0 4px' }}>Create New Campaign</Title>
        ...
        >
          <Space orientation="vertical" size={16} style={{ width: '100%' }}>
              ...
              <Row gutter={[16, 16]}>
                ...
                <Col xs={24} md={8}>
                  <Form.Item name="parentId" label="Parent Campaign ID">
                    <Select options={CAMPAIGN_OPTIONS} placeholder="Select..." allowClear />
                  </Form.Item>
                </Col>
            ...
            <Card
              ...
              styles={{ header: {} }}
            >
              <Row align="middle" justify="space-between" style={{ marginBottom: featureCard ? 16 : 0 }}>
                <Col>
                  <Text strong style={{ fontSize: 13 }}>Campaign Feature Card</Text>
                  <br />
                  <Text type="secondary" style={{ fontSize: 12 }}>
                    Enable prominent placement in app discovery feed
                  </Text>
                </Col>
                <Col>
                ...
                <Row gutter={[16, 16]}>
                  <Col xs={24}>
                    <Form.Item
                      ...
                      validateStatus={pageIdStatus === 'error' ? 'error' : pageIdStatus === 'success' ? 'success' : ''}
                      help={
                        pageIdStatus === 'success' ? 'ID format is valid' :
                        pageIdStatus === 'error' ? 'Invalid format. Expected: PID_XXXXX_XXXX' : undefined
                      }
                    ...
                    <Button type="link" size="small" icon={<LinkOutlined />} style={{ paddingLeft: 0, marginTop: -8 }}>
              ...
              <Row gutter={[16, 16]}>
                ...
                <Col xs={24} md={16}>
                  ...
                  >
                    <Row gutter={[16, 0]}>
                      <Col span={12}>
                        <Form.Item name="klookPct" noStyle rules={[{ required: true, message: '' }]}>
                          <Space.Compact style={{ width: '100%' }}>
                ...
                <Col xs={24}>
                  <Form.Item name="brandNotes" label="Brand Awareness Notes">
                    <TextArea rows={3} placeholder="Describe the qualitative brand impact goals..." />
                  </Form.Item>
                </Col>
              </Row>
            </Card>
          ...
          {/* ── Footer ────────────────────────────────────────────────────── */}
          <div className="cpb-antd-footer">
            <Button type="link" onClick={() => form.resetFields()} style={{ paddingLeft: 0 }}>
              Cancel changes
            </Button>
            <Space>
              <Button onClick={handleSaveDraft}>Save as Draft</Button>
              <Button type="primary" htmlType="submit" icon={<ArrowRightOutlined />} iconPlacement="end">
      ...
      <Modal
        ...
        footer={null}
        width={420}
        title={<Space><ExclamationCircleFilled style={{ color: '#D85A30' }} />S-Level Campaign Conflict</Space>}
      >
        <Space orientation="vertical" style={{ width: '100%' }} size={12}>
          <Text type="secondary">This campaign overlaps with an existing S-level campaign:</Text>
          <Alert
            type="warning"
          ...
          <Row gutter={[16, 0]} style={{ marginTop: 4 }}>
            <Col span={12}><Button block onClick={() => setConflictVisible(false)}>Go Back</Button></Col>
            <Col span={12}>
              <Button block danger onClick={() => { setConflictVisible(false); setView('success') }}>
                Ignore & Save
              </Button>
            </Col>
          </Row>
